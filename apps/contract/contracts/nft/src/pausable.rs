use soroban_sdk::{
    contracttype, Address, Env, Symbol, symbol_short,
};
...
impl NFTPausable {
    ...
    pub fn initialize(env: &Env) {
        ...
        env.storage().instance().set(&PauseOperation::MintPause, &false);
        env.storage().instance().set(&PauseOperation::TransferPause, &false);
        env.storage().instance().set(&PauseOperation::MetadataPause, &false);
        env.storage().instance().set(&PauseOperation::GlobalPause, &false);
    }
    
    // Check if an operation is paused
    pub fn is_paused(env: &Env, operation: &PauseOperation) -> bool {
        // Global pause overrides all other pause states
        let global_paused: bool = env.storage()
            .instance()
    ...
    pub fn unpause(env: &Env, operation: &PauseOperation, account: &Address) -> Result<(), NFTError> {
        // Check if the caller has admin role
        AccessControl::require_role(env, DEFAULT_ADMIN_ROLE, account)?;
