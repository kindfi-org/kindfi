use soroban_sdk::{
    contracttype, Address, Env, Vec, Map, symbol_short, Symbol,
};
...
impl RateLimiter {
    ...
    pub fn set_limit(env: &Env, operation: Symbol, limit: u32, window_ledgers: u32) {
        env.storage().instance().set(&RateLimitDataKey::Limits(operation.clone()), &limit);
        env.storage().instance().set(&RateLimitDataKey::WindowDuration(operation), &window_ledgers);
    }
    ...
    pub fn check_rate_limit(env: &Env, user: &Address, operation: Symbol) -> Result<(), NFTError> {
        ...
        let limit: Option<u32> = env.storage().instance().get(&RateLimitDataKey::Limits(operation.clone()));
        ...
        let window_ledgers: u32 = env.storage()
            .instance()
        ...
        let last_operation: Option<u32> = env.storage()
            .instance()
            .get(&RateLimitDataKey::UserLastOperation(user.clone(), operation.clone()));
            ...
            Some(last_ledger) => {
                ...
                if current_ledger < last_ledger + window_ledgers {
                    ...
                    let current_count: u32 = env.storage()
                        .instance()
                        .get(&RateLimitDataKey::UserOperationCount(user.clone(), operation))
                        .unwrap_or(0);
