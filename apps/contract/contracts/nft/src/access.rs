use soroban_sdk::{
    contracttype, Address, Env, Vec, symbol_short, Symbol, vec,
    auth::{ContractContext}, xdr::Signature,
};
...
impl AccessControl {
    ...
    pub fn initialize(env: &Env, admin: &Address) {
        ...
        env.storage().instance().set(&AccessDataKey::RoleMembers(DEFAULT_ADMIN_ROLE), &role_members);
        
        // Set admin roles (default_admin has all roles)
        let admin_roles: Vec<u32> = vec![env, DEFAULT_ADMIN_ROLE];
        env.storage().instance().set(&AccessDataKey::MemberRoles(admin.clone()), &admin_roles);
        
        // Set initial thresholds
        env.storage().instance().set(&AccessDataKey::MultiSigThreshold(DEFAULT_ADMIN_ROLE), &2u32); // Default: require 2 admins
        
        // Set admin role as admin for all roles
        env.storage().instance().set(&AccessDataKey::RoleAdminConfig(DEFAULT_ADMIN_ROLE), &DEFAULT_ADMIN_ROLE);
        env.storage().instance().set(&AccessDataKey::RoleAdminConfig(MINTER_ROLE), &DEFAULT_ADMIN_ROLE);
        env.storage().instance().set(&AccessDataKey::RoleAdminConfig(METADATA_MANAGER_ROLE), &DEFAULT_ADMIN_ROLE);
    }

    // Check if an address has a role
    pub fn has_role(env: &Env, role: u32, account: &Address) -> bool {
        let role_members: Vec<Address> = env.storage().instance()
    ...
    pub fn grant_role(env: &Env, role: u32, account: &Address) -> Result<(), NFTError> {
        // Update role members
        let mut role_members: Vec<Address> = env.storage()
            .instance()
            .get(&AccessDataKey::RoleMembers(role))
            .unwrap_or_else(|| Vec::new(env));
        ...
        if !role_members.contains(account) {
            ...
            env.storage().instance().set(&AccessDataKey::RoleMembers(role), &role_members);
        }
        
        // Update member roles
        let mut member_roles: Vec<u32> = env.storage()
            .instance()
        ...
        if !member_roles.contains(&role) {
            ...
            env.storage().instance().set(&AccessDataKey::MemberRoles(account.clone()), &member_roles);
        }
    ...
    pub fn set_multisig_threshold(env: &Env, role: u32, threshold: u32) -> Result<(), NFTError> {
        env.storage().instance().set(&AccessDataKey::MultiSigThreshold(role), &threshold);
        Ok(())
    }
    ...
    pub fn require_role_with_multiple_signatures(
        ...
        let role_members: Vec<Address> = env.storage()
            .instance()
            .get(&AccessDataKey::RoleMembers(role))
            .unwrap_or_else(|| Vec::new(env));
        ...
        for (signer, signature) in signatures.iter() {
            ...
            if role_members.contains(signer) {
                ...
                if env.crypto().ed25519_verify(signer, &payload.slice(0..), &signature) {
                    valid_sigs += 1;
                }
            }
        }
    ...
    fn specific_remove_role_member(env: &Env, role: u32, account: &Address) -> Result<(), NFTError> {
        let mut role_members: Vec<Address> = env.storage().instance()
            .get(&AccessDataKey::RoleMembers(role))
            .unwrap_or_else(|| Vec::new(env));
        ...
        if role_members.contains(account) {
            ...
            env.storage().instance().set(&AccessDataKey::RoleMembers(role), &new_members);
        }
    ...
    fn specific_remove_member_role(env: &Env, account: &Address, role: &u32) -> Result<(), NFTError> {
        let mut member_roles: Vec<u32> = env.storage().instance()
            .get(&AccessDataKey::MemberRoles(account.clone()))
            .unwrap_or_else(|| Vec::new(env));
        ...
        if member_roles.contains(role) {
            ...
            env.storage().instance().set(&AccessDataKey::MemberRoles(account.clone()), &new_roles);
        }
