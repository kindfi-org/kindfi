...
const GridContext = createContext<unknown>({})
...
const Grid = forwardRef<React.ElementRef<typeof View>, IGridProps>(
	({ className, _extra, children, ...props }, ref) => {
		...
		const responsiveNumColumns = useBreakpointValue(
			obj as {
				[key: string]: number
			},
		)
		...
		const itemsPerRow = useMemo(() => {
			...
			const colSpanArr = React.Children.map(children, (child: unknown) => {
				...
				if (colSpan > (responsiveNumColumns as number)) {
			...
			const rowItemsCount = arrangeChildrenIntoRows({
				childrenArray,
				colSpanArr,
				numColumns: responsiveNumColumns as number,
			})
		...
		const childrenWithProps = React.Children.map(children, (child, index) => {
			if (React.isValidElement(child)) {
				return React.cloneElement(child, { index } as unknown)
			}
	...
	({ className, _extra, ...props }, ref) => {
		...
		return (
			<View
				...
				style={[
					{
						flexBasis: flexBasisValue as unknown,
					},
					props.style,
				]}
			/>
