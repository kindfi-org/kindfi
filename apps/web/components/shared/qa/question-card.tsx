		...
		<Card className="question-card overflow-hidden">
			<CardHeader className="pb-3">
				<div className="flex justify-between items-start">
					<UserInfo authorData={question.author} createdAt={question.created_at as string} size="sm" />
				...
				{expanded && (
					<div className="w-full border-t pt-3 mt-2">
						{question.answers && question.answers.length > 0 ? (
							<div className="answer-list space-y-4 mb-6">
								{question.answers.map((answer) => (
									<div key={answer.id} className="mb-6">
										<div className={`answer-item pl-4 border-l-2 ${answer.author_id ? 'border-blue-300 bg-blue-50/30' : 'border-blue-100'} py-2 rounded-r-sm`}>
											<div className="flex items-start gap-2">
												<div className="flex-1">
													<UserInfo authorData={answer.author} createdAt={answer.created_at as string} size="sm" />
										...
										{answer.replies && answer.replies.length > 0 && (
											<div className="ml-8 mt-2">
												{answer.replies.map((reply) => (
													<div key={reply.id} className="pl-4 border-l-2 border-gray-100 py-2 mb-2 rounded-r-sm">
														<div className="flex-1">
															<UserInfo authorData={reply.author} createdAt={reply.created_at as string} size="sm" />
											...
											<div className="reply-form ml-8 mt-2 mb-4">
												<div className="pl-4 border-l-2 border-gray-100 py-2">
													...
													<div className="flex justify-end gap-2 mt-2">
														<Button size="sm" variant="outline" className="text-xs" onClick={onCancelReply} aria-label="Cancel reply">
															Cancel
														</Button>
														<Button
															size="sm"
							...
							<p className="text-muted-foreground text-sm mb-4">No answers yet. Be the first to answer!</p>
						)}
							...
							<div className="answer-form w-full border p-4 rounded-md bg-gray-50">
								...
								<div className="flex justify-end mt-2">
									<Button
										...
										disabled={!replyContent[question.id]?.trim() || !!submitAnswerPending || !!isGuestLimitReached}
