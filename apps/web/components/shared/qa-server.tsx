...
export default async function QA({ projectId, currentUser }: QAProps) {
  ...
  if (initialQuestions && initialQuestions.length > 0) {
    const authorIds = [...new Set(initialQuestions.map((item: any) => item.author_id))];
    ...
    const authorsMap = authors ? authors.reduce((acc: Record<string, UserData>, author: any) => {
  ...
  if (commentsData && commentsData.length > 0) {
    const authorIds = [...new Set(commentsData.map((item: any) => item.author_id))];
