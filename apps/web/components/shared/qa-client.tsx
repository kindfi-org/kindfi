...
const getGuestRemainingComments = () => {
  ...
  const count = Number.parseInt(localStorage.getItem('guestCommentCount') || '0', 10);
...
const incrementGuestCommentCount = () => {
  ...
  const currentCount = Number.parseInt(localStorage.getItem('guestCommentCount') || '0', 10);
...
export default function QAClient({ 
  ...
  const effectiveUser = currentUser || (guestUserId ? {
    id: guestUserId,
    ...
    async (supabase: TypedSupabaseClient) => {
      try {
        ...
        const { data, error } = await supabase
          .from("comments")
          .select("id, content, created_at, project_id, author_id, type, parent_comment_id, is_resolved")
        ...
        if (data && data.length > 0) {
          ...
          if (authors) {
            ...
            const authorsMap = authors.reduce((acc: Record<string, UserData>, author: any) => {
            ...
            return data.map((question: any) => ({
              ...
              author: authorsMap[question.author_id] || 
                (question.author_id?.includes('-') ? 
                  { id: question.author_id, full_name: "Guest User", is_team_member: false } : 
                  null)
            })) as CommentData[];
          }
        }
  ...
  useEffect(() => {
    ...
    try {
      const answers = commentsData.filter(comment => comment.type === 'answer');
      ...
      const questionsWithAnswers = questions.map(question => {
        const questionAnswers = answers
          ...
          .map(answer => {
            const answerReplies = replies.filter(reply => 
              reply.parent_comment_id === answer.id
            );
  ...
  const toggleRealtime = () => {
    ...
    setRealtimeStatus(isRealtimeEnabled ? 'Real-time updates disabled' : 'Real-time updates enabled');
    setTimeout(() => setRealtimeStatus(null), 3000);
  };
  ...
  const submitQuestionMutation = useMutation({
    ...
    onError: (error) => {
      ...
      setRealtimeStatus(`Error: ${error instanceof Error ? error.message : String(error)}`);
      setTimeout(() => setRealtimeStatus(null), 5000);
    }
  });
  ...
  const submitAnswerMutation = useMutation({
    mutationFn: async ({ questionId, answerContent }: { questionId: string; answerContent: string }) => {
      if (!effectiveUser?.id) {
        throw new Error("User ID is required");
      }
  ...
  const submitReplyMutation = useMutation({
    mutationFn: async ({ answerId, replyContent }: { answerId: string; replyContent: string }) => {
      if (!effectiveUser?.id) {
        throw new Error("User ID is required");
      }
  ...
  const renderUserInfo = (user: UserData | null, createdAt: string, size: "sm" | "md" = "md") => {
      ...
      <div className="flex items-center gap-2">
        ...
        <div>
          <div className="flex items-center">
            ...
            {user?.is_team_member && (
              <Badge variant="outline" className="ml-2 text-xs py-0 h-5 bg-blue-50 text-blue-700 border-blue-200">
                Team
              </Badge>
            )}
          </div>
  ...
  return (
    <>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button
                  ...
                  aria-label={isRealtimeEnabled ? "Disable real-time updates" : "Enable real-time updates"}
                >
                  {isRealtimeEnabled ? (
      ...
      <p className="text-gray-500 mb-1">
        Ask questions about this project and get answers from the community and project team members.
        {isRealtimeEnabled && <span className="text-blue-600 ml-1">Real-time updates are enabled.</span>}
      </p>
      
      
      {realtimeStatus && (
        <div className={`mb-4 py-2 px-3 text-sm rounded-md ${realtimeActivity ? 'bg-blue-50 text-blue-700' : 'bg-gray-50 text-gray-700'}`}>
          <div className="flex items-center gap-2">
            {realtimeActivity && <div className="w-2 h-2 bg-blue-500 rounded-full animate-pulse" />}
            {realtimeStatus}
          </div>
        </div>
      )}
      ...
      {!currentUser && guestRemainingComments > 0 && (
        <Alert variant="default" className="mb-6 bg-blue-50 text-blue-700 border-blue-200">
          <AlertTitle className="flex items-center gap-2">
            <UserIcon className="h-4 w-4" />
            Guest Mode
          </AlertTitle>
          <AlertDescription>
            You're currently browsing as a guest. You have {guestRemainingComments} comment{guestRemainingComments !== 1 ? 's' : ''} remaining. 
            <Button 
              variant="link" 
              onClick={handleGoToLogin} 
              className="p-0 h-auto text-blue-700 font-medium underline ml-1">
              Log in
            </Button> to participate without limits.
          </AlertDescription>
        </Alert>
      )}
      
      {!currentUser && guestRemainingComments === 0 && (
        <Alert variant="default" className="mb-6 bg-yellow-50 text-yellow-700 border-yellow-200">
          <AlertTitle className="flex items-center gap-2">
          ...
          <AlertDescription>
            ...
            <Button 
              ...
              Log in
            </Button> to continue participating or reset your guest session.
          </AlertDescription>
        </Alert>
      )}
        ...
        <Card className="border-0 shadow-sm">
          ...
          <CardFooter className="flex justify-end gap-2">
            <Button
              ...
              disabled={!newQuestion.trim() || submitQuestionMutation.isPending || (!currentUser && guestRemainingComments === 0)}
          ...
          {processedQuestions && processedQuestions.length > 0 ? (
            <div className="space-y-4">
              {processedQuestions.map((question) => (
                <Card key={question.id} className="question-card overflow-hidden">
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                      {renderUserInfo(question.author || null, question.created_at)}
                      <div className="flex items-center gap-2">
                  ...
                  <CardFooter className="flex flex-col items-start pt-0">
                    <div className="w-full flex justify-between items-center mb-3">
                      <Button
                        ...
                        aria-label={expandedQuestionIds[question.id] ? "Collapse answers" : "Expand answers"}
                      >
                          ...
                          <div className="answer-list space-y-4 mb-6">
                            {question.answers.map((answer) => (
                              <div key={answer.id} className="mb-6">
                                ...
                                <div className={`answer-item pl-4 border-l-2 ${answer.author?.is_team_member ? 'border-blue-300 bg-blue-50/30' : 'border-blue-100'} py-2 rounded-r-sm`}>
                                  <div className="flex items-start gap-2">
                                    <div className="flex-1">
                                      {renderUserInfo(answer.author || null, answer.created_at, "sm")}
                                      ...
                                      {replyingTo !== answer.id && effectiveUser && (
                                        <Button
                                          ...
                                          onClick={() => startReplyingTo(answer.id)}
                                          aria-label="Reply to this answer"
                                          disabled={!currentUser && guestRemainingComments === 0}
                                        >
                                ...
                                {answer.replies && answer.replies.length > 0 && (
                                  <div className="ml-8 mt-2">
                                    {answer.replies.map((reply) => (
                                      ...
                                      >
                                        <div className="flex-1">
                                          {renderUserInfo(reply.author || null, reply.created_at, "sm")}
      ...
      <Dialog open={isLoginDialogOpen} onOpenChange={setIsLoginDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              You've used all 3 comments available in guest mode. Would you like to log in or continue as a guest with a fresh session?
            </DialogDescription>
          </DialogHeader>
          <div className="flex justify-center my-4">
            <div className="bg-blue-50 p-4 rounded-lg text-center w-full">
              <p className="text-blue-700 font-medium mb-2">Create an account to:</p>
