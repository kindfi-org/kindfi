...
export function FoundationEscrowAdminClientWrapper({ foundationSlug }: { foundationSlug: string }) {
	...
	return (
		<ManagePageShell>
			<ManageSectionHeader
				icon={<IoSettingsOutline size={24} className="relative z-10" aria-hidden="true" />}
			...
			>
				<div className="p-6 rounded-lg border bg-card">
					<p className="text-muted-foreground">
						Foundation escrow management coming soon. The escrow system will be adapted to support
						foundations.
					</p>
					{escrowContract ? (
						<div className="mt-4 p-4 bg-muted rounded-lg">
							<p className="text-sm font-medium">Current Escrow Contract:</p>
							<p className="text-xs text-muted-foreground font-mono">{escrowContract.contractId}</p>
							<p className="text-xs text-muted-foreground">State: {escrowContract.currentState}</p>
						</div>
					) : null}
				</div>
			</motion.div>
