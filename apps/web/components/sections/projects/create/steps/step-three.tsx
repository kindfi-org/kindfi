...
export function StepThree({ onBack, onSubmit, isPending = false }: StepThreeProps) {
	...
	const canSubmit = locationValue?.length === 3 && categoryValue?.length > 0 && !isPending
								...
								render={({ field }) => (
									<FormItem className="flex flex-col space-y-2">
										...
										<p className="text-sm text-muted-foreground">
											Select the primary country or region where your project operates.
										</p>
									</FormItem>
								)}
							/>
								...
								render={({ field }) => (
									<FormItem className="space-y-3">
										...
										{!isLoading && !categoriesError && categories.length > 0 && (
											<p className="text-sm text-muted-foreground">
												Choose the category that best describes your project&apos;s primary focus.
											</p>
										)}
									</FormItem>
								)}
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem className="space-y-2">
										...
										<FormMessage />
										<p className="text-sm text-muted-foreground">
											Add up to 10 tags to help people discover your project. Press Enter to add a
											tag.
										</p>
									</FormItem>
								)}
