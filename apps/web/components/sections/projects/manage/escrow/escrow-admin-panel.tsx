...
export function EscrowAdminPanel({
  ...
  const handleCreateEscrow = async () => {
    try {
      ...
      const res = await deployEscrow(payload, (escrowType as EscrowType) || 'single-release')
      if (res.status !== 'SUCCESS') throw new Error('Failed to create escrow')
      toast.success('Escrow initialized. Sign and broadcast next steps in flow.')
    } catch (e) {
      console.error(e)
  ...
  const handleApproveMilestone = async () => {
    if (!escrowContractAddress) return toast.error('No escrow contract configured')
    try {
      const signer = await ensureWallet()
      ...
      toast.success('Milestone approved (unsigned tx). Continue to sign & submit.')
    } catch (e) {
      console.error(e)
  ...
  const handleChangeMilestoneStatus = async () => {
    ...
    try {
      ...
      toast.success('Milestone status updated (unsigned tx). Continue to sign & submit.')
    } catch (e) {
      console.error(e)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3">
        <label htmlFor={titleId} className="text-sm font-medium">Escrow Title</label>
        <Input id={titleId} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Project Escrow" />
      </div>
      ...
      <div className="flex gap-3">
        <Button onClick={handleCreateEscrow} className="text-white">Create Escrow</Button>
      </div>

      <div className="h-px bg-gray-200" />

      <div className="grid gap-3">
        <label htmlFor={milestoneId} className="text-sm font-medium">Milestone Index</label>
        <Input id={milestoneId} value={milestoneIndex} onChange={(e) => setMilestoneIndex(e.target.value)} />
      </div>

      <div className="flex gap-3">
        <Button variant="outline" onClick={handleApproveMilestone}>Approve Milestone</Button>
        <Button variant="outline" onClick={handleChangeMilestoneStatus}>Change Status</Button>
      </div>
    </div>
  )
}
