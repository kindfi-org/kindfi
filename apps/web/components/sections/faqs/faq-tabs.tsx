...
import { LayoutDashboard, Megaphone, Users, Shield, ChevronDown } from 'lucide-react'
			...
			<div className="w-full md:w-[40rem] lg:w-[55rem] xl:w-[65rem]">
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
					{(['platform','campaigns','donors','security'] as const).map((key) => {
						...
						const Icon = key === 'platform' ? LayoutDashboard : key === 'campaigns' ? Megaphone : key === 'donors' ? Users : Shield
						return (
							<button
								key={key}
								type="button"
							...
							>
								<span className={`inline-flex h-9 w-9 items-center justify-center rounded-xl ${isActive ? 'bg-white/10' : 'bg-gray-100'} transition`}>
								...
								<span className="flex-1 text-left">
									...
									<span className={`mt-1 block text-xs ${isActive ? 'text-white/80' : 'text-[#727276]' }`}>
				...
				<div className="mt-8">
					<h2 className="text-black font-semibold text-xl md:text-2xl">
						{(t(`faqs.categories.${activeTab}.title`) === `faqs.categories.${activeTab}.title`) ? categoryTitles[activeTab].title : t(`faqs.categories.${activeTab}.title`)}
					</h2>
					<p className="text-[#727276] text-sm md:text-base">
						{(t(`faqs.categories.${activeTab}.subtitle`) === `faqs.categories.${activeTab}.subtitle`) ? categoryTitles[activeTab].subtitle : t(`faqs.categories.${activeTab}.subtitle`)}
					</p>
					<div className="w-full mt-5 space-y-3">
						{faqData[activeTab].map(({ id, question, answer }) => {
							...
							return (
								<div key={id} className="border border-[#E6E6E6] rounded-2xl bg-white shadow-sm overflow-hidden">
									<button
										type="button"
									...
									<div className={`px-4 md:px-6 ${isOpen ? 'pt-3 md:pt-4 pb-4 md:pb-6' : 'pb-0'} text-[#4B5563] text-xs md:text-sm leading-relaxed border-t border-[#F0F0F0] transition-all duration-300 ${isOpen ? 'opacity-100 max-h-[1000px]' : 'opacity-0 max-h-0 overflow-hidden'}`}>
										{translatedAnswer}
									</div>
								</div>
							)
