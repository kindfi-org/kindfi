...
export async function GET(_request: NextRequest, context: TagRouteContext) {
	try {
		...
		const { data, error } = await supabase.from('project_tags').select('*').eq('id', tagId).single()
...
export async function DELETE(_request: NextRequest, context: TagRouteContext) {
	try {
		...
		const { data, error } = await supabase.from('project_tags').delete().eq('id', tagId).select()
