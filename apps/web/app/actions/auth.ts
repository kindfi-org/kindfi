...
export async function signUpAction(formData: FormData): Promise<AuthResponse> {
  ...
  if (existingUser) {
    console.warn('🔧 KindfiSupabaseAdapter: User already exists', existingUser.id)
...
export async function requestResetAccountAction(formData: FormData): Promise<void> {
  ...
  try {
    ...
    const response = errorHandler.handleAuthError(error as AuthError, 'reset_account')
...
export async function checkAuthStatus(): Promise<AuthResponse> {
  ...
  try {
    ...
    if (!session) {
      return { success: false, message: 'No active session', redirect: '/sign-in' }
    }
...
export async function updateEscrowStatusAction(
  ...
  try {
    ...
    logger.error({ eventType: 'ESCROW_STATUS_UPDATE_ERROR', error, id, newStatus })
    return {
      success: false,
...
export async function updateEscrowMilestoneAction(
  ...
  try {
    ...
    logger.error({ eventType: 'ESCROW_MILESTONE_UPDATE_ERROR', error, id, current, completed })
    return {
      success: false,
...
export async function updateEscrowFinancialsAction(
  ...
  try {
    ...
    logger.error({ eventType: 'ESCROW_FINANCIALS_UPDATE_ERROR', error, id, funded, released })
    return {
      success: false,
...
export async function insertTestEscrowRecordAction(): Promise<EscrowResponse> {
  ...
  try {
    ...
    revalidatePath('/admin/escrow')
    return { success: true, message: 'Test record inserted successfully', data }
  } catch (error) {
...
export async function updateDeviceWithDeployee(deployeeUpdateData: string) {
  const { deployeeAddress, aaguid, userId, credentialId }: {
  ...
  try {
    ...
    if (!userId || !credentialId || !deployeeAddress || !aaguid) {
      return { success: false, message: 'Missing required parameters', error: 'Invalid input parameters' }
    }

    const existingDevice = await db
      .select({ id: devices.id, userId: devices.userId, aaguid: devices.aaguid, credentialId: devices.credentialId })
      .from(devices)
      .where(and(eq(devices.userId, userId), eq(devices.credentialId, credentialId)))
      .limit(1)

    if (!existingDevice.length) {
      return { success: false, message: 'Device not found or does not belong to user', error: 'Device verification failed' }
    }
    ...
    const updatedDevice = await db
      .update(devices)
      .set({ address: deployeeAddress, aaguid: aaguid || deviceToUpdate.aaguid, updatedAt: new Date().toISOString() })
      .where(eq(devices.id, deviceToUpdate.id))
      .returning()

    if (!updatedDevice.length) {
      logger.error({ eventType: 'DEVICE_UPDATE_ERROR', error: 'No device was updated', userId, credentialId })
      return { success: false, message: 'Failed to update device information', error: 'No device was updated' }
    }

    logger.info({ eventType: 'DEVICE_UPDATED', userId, credentialId, deployeeAddress, aaguid })

    return { success: true, message: 'Device updated successfully', data: updatedDevice[0] }
  } catch (error) {
    logger.error({ eventType: 'DEVICE_UPDATE_EXCEPTION', error, userId, credentialId })
    return { success: false, message: 'An error occurred while updating the device', error: error instanceof Error ? error.message : 'Unknown error' }
  }
}
