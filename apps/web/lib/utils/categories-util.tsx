...
/**
 * Renders an animated category badge (for interactive components)
 */
export function RenderAnimatedCategory({
	category,
	onClick,
}: {
	...
	const categoryName = typeof category === 'string' ? category : category.label
	const categoryIcon = typeof category === 'string' ? undefined : category.icon
	const { color, icon } = getCategoryStyles(categoryName)
	...
	return (
		...
		>
			<Badge
				variant="secondary"
				className={`px-3 py-1.5 rounded-full ${color} shadow-sm border border-transparent hover:shadow-md cursor-pointer`}
			>
				{/* Use the category's own icon if provided, otherwise use the mapped icon */}
				<span className="mr-1.5">{categoryIcon || icon}</span>
