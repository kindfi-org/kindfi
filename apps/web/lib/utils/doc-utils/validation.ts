interface ExtractedData {
  ...
  // Define types for the Tesseract progress/logger message
  ...
  export const processFile = async (
    ...
    Tesseract: {
      recognize: (
        arg0: File,
        arg1: string,
        arg2: { logger: (message: TesseractProgressMessage | unknown) => void },
    ...
    try {
      const result = await Tesseract.recognize(file, 'eng', {
        logger: (message: unknown) => {
          // Type guard function for TesseractProgressMessage
          const isProgressMessage = (msg: unknown): msg is TesseractProgressMessage => {
            return (
              typeof msg === 'object' && 
              msg !== null && 
              'status' in msg && 
          ...
          if (isProgressMessage(message) && message.status === 'recognizing text') {
