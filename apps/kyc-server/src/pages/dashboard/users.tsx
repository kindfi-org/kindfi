...
import { Card, CardHeader, CardTitle, CardDescription } from '~/components/base/card'
...
export default function Users() {
		...
		onUpdate: (update) => {
			setAllUsers(currentUsers => {
				const updatedUsers = currentUsers.map(user => 
					user.user_id === update.data.user_id || user.id === update.data.user_id
						? { ...user, status: update.data.status, verification_level: update.data.verification_level }
						: user
				)
		...
		try {
			...
			if (data.users) {
				...
				setStats({
					...
					verified: data.users.filter(u => u.status === 'verified').length
				})
			}
		} catch (err) {
			...
			setStats({
				...
				pending: mockKycRecords.filter(u => u.status === 'pending').length,
				approved: mockKycRecords.filter(u => u.status === 'approved').length,
				rejected: mockKycRecords.filter(u => u.status === 'rejected').length,
				verified: mockKycRecords.filter(u => u.status === 'verified').length
			})
		}
		setLoading(false)
	}
	...
	useEffect(() => {
		...
		const filtered = allUsers.filter(user => 
			user.user_id?.includes(search) || user.id?.includes(search)
		)
			...
			<div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
				...
				<div className="grid grid-cols-2 @xl/main:grid-cols-3 @5xl/main:grid-cols-5 gap-4 px-4 lg:px-6 mb-6">
					...
					<Card className="bg-gradient-to-t from-orange-100/20 to-card">
						<CardHeader>
							<CardDescription className="text-orange-600">Pending</CardDescription>
					...
					<Card className="bg-gradient-to-t from-blue-100/20 to-card">
						<CardHeader>
							<CardDescription className="text-blue-600">Verified</CardDescription>
